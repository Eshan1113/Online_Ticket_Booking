...
if(isset($_POST['flight_but'])) {
    ...
    if($dep_city===$arr_city || $arr_city==='To' || $arr_city==='From') {
      header('Location: filightadd.php?error=same');
      exit();
    }
    ...
    if($flag == false) {
      header('Location: filightadd.php?error=destless');
      exit();
    } else {
      ...
      if($row = mysqli_fetch_assoc($result)) {
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)) {
          header('Location: filightadd.php?error=sqlerr1');
          exit();          
        } else {      
        ...
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
        header('Location: filightadd.php?flight=success');
        exit();
      } else {
        header('Location: filightadd.php?error=sqlerr');
        exit();
      }
    }
} else {
    header('Location: filightadd.php');
    exit();
}
